...
function getCompletion(namespace, pattern, strip = pattern.length) {
    return exports.store.completion[namespace]
        ?.filter(e => e.key.startsWith(pattern))
        .map(e => (0, langData_1.convertToCompletion)(e, strip)) ?? [];
}
exports.getCompletion = getCompletion;
function getDocumentationSingle(namespace, name) {
    const found = exports.store.completion[namespace]?.find(e => e.key === name);
    if (found !== void 0 && found !== null) {
        return (0, langData_1.makeDocumentation)(found, 0);
    }
    return null;
}
exports.getDocumentationSingle = getDocumentationSingle;
function getSourceData(namespace, name) {
    const found = exports.store.completion[namespace]?.find(e => e.key === name);
    if (found !== void 0 && found !== null) {
        return found;
    }
...
exports.flushNamespace = flushNamespace;
function syncNamespace(namespace, file, data) {
    const flatData = data.toFlatData();
    const completions = Object.keys(flatData);
    flushNamespace(namespace, file);
    exports.store.completion[namespace] = completions.map(completion => {
        return {
            key: completion,
            file: file,
            metadata: data,
...
exports.syncNamespace = syncNamespace;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CONF_NAMESPACE = exports.LANG_NAMESPACE = void 0;
exports.LANG_NAMESPACE = "LANG_NAMESPACE";
exports.CONF_NAMESPACE = "CONF_NAMESPACE";
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.convertToCompletion = exports.makeDocumentation = exports.getLastActivePart = exports.RAW_SUPPORT_TYPES = void 0;
const path_1 = __webpack_require__(1);
const vscode_1 = __webpack_require__(2);
exports.RAW_SUPPORT_TYPES = new Set(["string", "number", "boolean"]);
function getLastActivePart(name, strip = 0) {
...
exports.getLastActivePart = getLastActivePart;
function makeDocumentation(data, strip = 0) {
    return new vscode_1.MarkdownString("### Ключ " + (strip === 0 ? "" : "...") + data.key.substring(strip) + "\n" +
        "Ключ: **" + data.key + "**\n\n" +
        "Файл: **" + (0, path_1.basename)(data.file) + "**\n\n\n" +
        "```\n" + (exports.RAW_SUPPORT_TYPES.has(typeof data.locale) ?
        data.locale :
        JSON.stringify(data.locale)) + "```", false);
}
exports.makeDocumentation = makeDocumentation;
function convertToCompletion(data, strip = 0) {
    const stripped = getLastActivePart(data.key, strip);
    const item = new vscode_1.CompletionItem({ label: stripped, description: (0, path_1.basename)(data.file) }, vscode_1.CompletionItemKind.Constant);
    item.insertText = stripped;
    item.documentation = makeDocumentation(data, strip);
    return item;
}
exports.convertToCompletion = convertToCompletion;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.extractPositionFromContents = exports.extractNotNull = exports.scanFiles = exports.objectFlatten = exports.exists = exports.getPathsToNamespace = exports.CONF_REGEXP = exports.LANG_REGEXP = void 0;
const fs = __webpack_require__(8);
const path_1 = __webpack_require__(1);
const vscode_1 = __webpack_require__(2);
const processors_1 = __webpack_require__(9);
const store_1 = __webpack_require__(4);
exports.LANG_REGEXP = /(?<=Lang\.get\(['"]|Lang\.getDefault\(['"])([^'"]*)/g;
exports.CONF_REGEXP = /(?<=Config\.get\(['"]|Config\.getDefault\(['"])([^'"]*)/g;
function getPathsToNamespace(namespace) {
    const configuration = vscode_1.workspace.getConfiguration('burmistr_react_components_helper');
    switch (namespace) {
        case "LANG_NAMESPACE": return configuration.fileLangsPaths;
        case "CONF_NAMESPACE": return configuration.fileConfigsPaths;
    }
}
exports.getPathsToNamespace = getPathsToNamespace;
...
function objectFlatten(input, stack = [], commonPrefix = '') {
    const entries = [];
...
async function scanFiles(namespace, flies = []) {
    const finalFiles = flies.length === 0 ? getPathsToNamespace(namespace) : flies;
    for (const processor of processors_1.default) {
        for (const file of finalFiles) {
            if ((0, path_1.isAbsolute)(file)) {
                ...
                for (const folder of vscode_1.workspace.workspaceFolders ?? []) {
                    const normalized = (0, path_1.join)(folder.uri.fsPath, file);
...
function extractNotNull(...items) {
    return items.find(e => e !== null) ?? null;
}
exports.extractNotNull = extractNotNull;
function extractPositionFromContents(file, position) {
    let line = 1;
    let last = 0;
    let current = 0;
...
exports.extractPositionFromContents = extractPositionFromContents;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const RawDataProcessor_1 = __webpack_require__(10);
exports["default"] = [new RawDataProcessor_1.default()];
...
exports["default"] = RawDataProcessor;
...
module.exports = __webpack_require__(13).YAML
...
var parseCst = __webpack_require__(14);
var Document$1 = __webpack_require__(16);
var Schema = __webpack_require__(18);
var PlainValue = __webpack_require__(15);
var warnings = __webpack_require__(19);
...
exports.JsonMetadata = JsonMetadata;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.ExtendedDocumentLink = void 0;
const vscode_1 = __webpack_require__(2);
const store_1 = __webpack_require__(4);
const support_1 = __webpack_require__(7);
class ExtendedDocumentLink extends vscode_1.DocumentLink {
    constructor(data, range) {
        super(range);
        this.data = data;
    }
}
exports.ExtendedDocumentLink = ExtendedDocumentLink;
class DocumentLinkProvider {
    provideInternal(namespace, matches, line, links) {
        if (matches !== null) {
    ...
    resolveDocumentLink(link, token) {
        ...
        const positions = (0, support_1.extractPositionFromContents)(data.file, position);
        const uri = vscode_1.Uri.parse("vscode://file/" + data.file + ":" + positions.line + ":" + positions.rest);
        link.target = uri;
        return link;
    }
    provideDocumentLinks(document, token) {
        ...
        while (lineNumber < document.lineCount) {
            ...
            this.provideInternal("LANG_NAMESPACE", line.text.match(support_1.LANG_REGEXP), line, links);
            this.provideInternal("CONF_NAMESPACE", line.text.match(support_1.CONF_REGEXP), line, links);
            lineNumber++;
        }
        return links;
    }
...
class HoverProvider {
    provideInternals(namespace, linkRange, document) {
        ...
        const text = document.getText(linkRange);
        const completion = (0, store_1.getDocumentationSingle)(namespace, text);
        if (completion !== null) {
            return new vscode_1.Hover(completion, linkRange);
        }
        else {
            return null;
        }
...
function activate(context) {
    ...
    context.subscriptions.push(vscode_1.workspace.onDidSaveTextDocument((document) => {
        ...
        (0, support_1.scanFiles)("CONF_NAMESPACE");
    }));
    (0, support_1.scanFiles)("LANG_NAMESPACE");
    (0, support_1.scanFiles)("CONF_NAMESPACE");
}
